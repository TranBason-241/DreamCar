/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package sontb.dao;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import org.apache.log4j.Logger;
import sontb.utils.MyConnection;

/**
 *
 * @author ADMIN
 */
public class OrderDAO {

static final Logger LOGGER = Logger.getLogger(OrderDAO.class);
    public int deleteOrder(String orderID) throws SQLException {
        Connection cn = null;
        int rs = -1;
        PreparedStatement pst = null;

        try {
            cn = MyConnection.getConnection();
            if (cn != null) {
                String sql = "UPDATE dbo.tblOrderss SET status ='inactive'\n"
                        + "WHERE orderID = '" + orderID + "'";
                pst = cn.prepareStatement(sql);
                rs = pst.executeUpdate();
            }
        } catch (Exception e) {
            LOGGER.error("error: ", e);
        } finally {
            if (pst != null) {
                pst.close();
            }
            if (cn != null) {
                cn.close();
            }
        }
        return rs;
    }

    public int SetStatusFeedBack(String orderID) throws SQLException {
        Connection cn = null;
        int rs = -1;
        PreparedStatement pst = null;

        try {
            cn = MyConnection.getConnection();
            if (cn != null) {
                String sql = "UPDATE dbo.tblOrderss SET statusFeedBack = 'done'\n"
                        + "  WHERE orderID = '"+orderID+"';";
                pst = cn.prepareStatement(sql);
                rs = pst.executeUpdate();
            }
        } catch (Exception e) {
            LOGGER.error("error: ", e);
        } finally {
            if (pst != null) {
                pst.close();
            }
            if (cn != null) {
                cn.close();
            }
        }
        return rs;
    }

}
